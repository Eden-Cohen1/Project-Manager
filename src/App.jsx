...
function App() {
  ...
  const handleDeleteProject = function () {
    const newProjectsArray = projectList.filter(
      (project) => project.id !== projectList[currentProjectIndex].id
    );
  ...
  return (
    <main className="flex h-full min-h-[100vh] pt-[5%]">
      <Sidebar
        ...
        onAddProjectSection={openAddProject}
        projectList={projectList}
        currentProject={projectList && projectList[currentProjectIndex]?.id}
      />
      {currentSection === "add-project" && (
        <section className="w-full flex items-top justify-center mb-[10%] mt-[10%]">
          <AddProject
            onAddProject={handleAddProject}
            onCloseAddProject={closeAddProject}
          />
        </section>
      )}
      {currentSection === "no-project" && (
        <NoProjects onAddProjectSection={openAddProject} />
      )}
      {currentSection === "project-edit" && (
        <section className="w-full flex items-top mt-[10%] mb-[10%]">
          <ProjectEdit
            project={projectList && projectList[currentProjectIndex]}
